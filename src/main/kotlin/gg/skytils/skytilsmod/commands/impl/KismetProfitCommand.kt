...
object KismetProfitCommand : BaseCommand("kismetprofit", aliases = listOf("kismet")) {
    ...
    override fun processCommand(player: EntityPlayerSP, args: Array<String>) {
        ...
        for (drop in seventhLoot) {
            ...
            var itemProfit = ((AuctionData.lowestBINs[drop.skyblockItemID] ?: 0.0) - drop.chestPrice) * dropChance / 100.0
            if (itemProfit <= 0.0) {
                itemProfit = 0.0
            }
        ...
        if (kismetPrice == 0.0) {
            ...
            UChat.chat("$successPrefix §fAverage profit per reroll is currently: §a${NumberUtil.nf.format(averageProfit)}" + if (args.isEmpty()) " §b(F7)" else (" §b(M)"))}
            else {
